<?php
	...
	if(isset($busqueda) && $busqueda!=""){
		...
		$consulta_datos="SELECT * FROM categoria WHERE categoria_nombre LIKE '%$busqueda%' OR 
        categoria_ubicacion LIKE '%$busqueda%' ORDER BY categoria_nombre ASC LIMIT $inicio,$registros";

		$consulta_total="SELECT COUNT(categoria_id) FROM categoria WHERE categoria_nombre LIKE '%$busqueda%' OR 
        categoria_ubicacion LIKE '%$busqueda%'";
	...
	if($total>=1 && $pagina<=$Npaginas){    //substr($rows['categoria_ubicacion'],0,25) sirve para mostrar una cantidad máxima de caracteres
		$contador=$inicio+1;
		$pag_inicio=$inicio+1;
